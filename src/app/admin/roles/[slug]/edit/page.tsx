...
function RoleEditFields({ slug, role }: { slug: string; role: RoleDetail }) {
  ...
  const [progressionEligible, setProgressionEligible] = useState(role.progressionEligible ?? false);
  const [teamRequirement, setTeamRequirement] = useState<"none" | "optional" | "required">(
    role.teamRequirement ?? "none"
  );
  const [teamRoleRequirement, setTeamRoleRequirement] = useState<"any" | "lead_or_owner">(
    role.teamRoleRequirement ?? "any"
  );
  const [requiresContribution, setRequiresContribution] = useState(
    role.requiresContribution ?? false
  );
