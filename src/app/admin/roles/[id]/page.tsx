    ...
    <div className="space-y-8">
      ...
      <div className="flex items-start justify-between gap-6">
        <div>
          ...
          {role.description && (
            <p className="font-mono text-xs text-zinc-500 mt-2 max-w-2xl">{role.description}</p>
          )}
        </div>
      ...
      <div className="bg-white border border-zinc-200">
        ...
        <div className="px-6 py-6">
          {role.permissions.length === 0 ? (
            <p className="font-mono text-xs text-zinc-400">No permissions assigned to this role.</p>
          ) : (
      ...
      <div className="bg-white border border-zinc-200">
        ...
        {role.members.length === 0 ? (
          <div className="px-6 py-6">
            <p className="font-mono text-xs text-zinc-400">No members assigned to this role yet.</p>
          </div>
        ) : (
          <div className="divide-y divide-zinc-100">
            ...
            {role.members.map((member) => (
              ...
              >
                <span className="font-sans text-sm text-zinc-900 truncate">{member.fullName}</span>
                <span className="font-mono text-xs text-zinc-500 truncate">
                  {member.communityId}
                </span>
                <span className="font-mono text-xs text-zinc-500 truncate">@{member.username}</span>
                <Link
